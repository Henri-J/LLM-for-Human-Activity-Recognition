...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        self.gemma = Gemma3ForCausalLM.from_pretrained(
            ...
            torch_dtype=torch.bfloat16 if configs.use_amp else torch.float32,
        )
        self.hidden_dim_of_gemma = 640  # From config.json
        ...
        if configs.mlp_hidden_layers == 0:
            ...
            self.encoder = nn.Linear(self.token_len, self.hidden_dim_of_gemma)
            self.decoder = nn.Linear(self.hidden_dim_of_gemma, 6)
        else:
            print("use mlp as tokenizer and detokenizer")
            self.encoder = MLP(
                self.token_len,
                self.hidden_dim_of_gemma,
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        means = x_enc.nanmean(1, keepdim=True).detach()
        x_enc = x_enc - means
        ...
        # Crucial for Gemma:
        features_embeds = features_embeds * (640**0.5)
        # Cast to BF16 to match Gemma's weights
        features_embeds = features_embeds.to(torch.bfloat16)
        outputs = self.gemma.model(inputs_embeds=features_embeds)[0]
        outputs = outputs.to(torch.float16)

        dec_out = self.decoder(outputs)

        # Average dec_out
        dec_out = dec_out.nanmean(dim=1)
        # Reshape based on actual number of variables (3: x, y, z)
        current_batch = dec_out.shape[0] // n_vars
        dec_out = dec_out.view(current_batch, n_vars, 6).nanmean(dim=1)
